...
impl AppMetrics {
    ...
    pub fn render(&self, config: &AppConfig) -> String {
        ...
        write_counter(
            ...
            self.secrets_created.load(R),
        );
        write_counter(
            &mut buf,
            "psst_secrets_read_total",
            "Total secrets read.",
            self.secrets_read.load(R),
        );
        write_counter(
            &mut buf,
            "psst_secrets_expired_total",
            "Total secrets deleted by expiry maintenance.",
            self.secrets_expired.load(R),
        );
        write_counter(
            &mut buf,
            "psst_secrets_deleted_total",
            "Total secrets deleted by owner.",
            self.secrets_deleted.load(R),
        );
        write_counter(
            &mut buf,
            "psst_abuse_reports_total",
            "Total abuse reports submitted.",
            self.abuse_reports.load(R),
        );

        let _ = writeln!(
            buf,
            "# HELP psst_create_rejected_total Total create requests rejected by reason."
        );
        let _ = writeln!(buf, "# TYPE psst_create_rejected_total counter");
        let _ = writeln!(
            buf,
            "psst_create_rejected_total{{reason=\"too_large\"}} {}",
            self.create_rejected_too_large.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_create_rejected_total{{reason=\"invalid_ttl\"}} {}",
            self.create_rejected_invalid_ttl.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_create_rejected_total{{reason=\"turnstile_failed\"}} {}",
            self.create_rejected_turnstile_failed.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_create_rejected_total{{reason=\"rate_limited\"}} {}",
            self.create_rejected_rate_limited.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_create_rejected_total{{reason=\"storage_limit\"}} {}",
            self.create_rejected_storage_limit.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_create_rejected_total{{reason=\"invalid_payload\"}} {}",
            self.create_rejected_invalid_payload.load(R)
        );

        let _ = writeln!(
            buf,
            "# HELP psst_rate_limited_total Total requests rejected by rate limiter."
        );
        let _ = writeln!(buf, "# TYPE psst_rate_limited_total counter");
        let _ = writeln!(
            buf,
            "psst_rate_limited_total{{operation=\"create\"}} {}",
            self.rate_limited_create.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_rate_limited_total{{operation=\"read\"}} {}",
            self.rate_limited_read.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_rate_limited_total{{operation=\"report\"}} {}",
            self.rate_limited_report.load(R)
        );

        let _ = writeln!(
            buf,
            "# HELP psst_db_errors_total Total database errors by operation."
        );
        let _ = writeln!(buf, "# TYPE psst_db_errors_total counter");
        let _ = writeln!(
            buf,
            "psst_db_errors_total{{operation=\"create\"}} {}",
            self.db_errors_create.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_db_errors_total{{operation=\"read\"}} {}",
            self.db_errors_read.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_db_errors_total{{operation=\"delete\"}} {}",
            self.db_errors_delete.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_db_errors_total{{operation=\"purge\"}} {}",
            self.db_errors_purge.load(R)
        );
        let _ = writeln!(
            buf,
            "psst_db_errors_total{{operation=\"metrics\"}} {}",
            self.db_errors_metrics.load(R)
        );

        write_counter(
            &mut buf,
            "psst_purge_runs_total",
            "Total maintenance purge runs.",
            self.purge_runs.load(R),
        );
        write_counter(
            &mut buf,
            "psst_purge_deleted_total",
            "Total expired secrets deleted by purge runs.",
            self.purge_deleted.load(R),
        );

        write_gauge_i64(
            &mut buf,
            "psst_active_secrets",
            "Number of currently active (unexpired, unconsumed) secrets.",
            self.active_secrets.load(R),
        );
        write_gauge_i64(
            &mut buf,
            "psst_storage_bytes",
            "Total storage bytes used by active secrets.",
            self.storage_bytes.load(R),
        );

        write_gauge(
            &mut buf,
            "psst_config_max_secret_bytes",
            "Configured maximum secret size in bytes.",
        ...
        write_gauge(
            ...
            "Whether secret creation is enabled (1=yes, 0=no).",
            u64::from(config.enable_create),
        );
